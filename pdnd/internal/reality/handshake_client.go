...
func (c *Conn) makeClientHello() (*clientHelloMsg, *keySharePrivateKeys, *echClientContext, error) {
	...
	if len(config.PublicKey) > 0 {
		if c.config.EncryptedClientHelloConfigList != nil {
			return nil, nil, nil, errors.New("REALITY: Encrypted ClientHello cannot be combined with native REALITY")
		}
		var authKey []byte
		authKey, err = applyRealityClientHello(hello, keyShareKeys, config)
		if err != nil {
			return nil, nil, nil, err
		}
		// The key is installed on the connection before certificate processing so
		// the fail-closed REALITY verifier can authenticate the synthetic cert.
		c.AuthKey = authKey
		c.installRealityClientVerifier()
	}
