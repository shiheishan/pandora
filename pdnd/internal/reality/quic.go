...
func QUICServer(config *QUICConfig) *QUICConn {
	if config == nil || config.TLSConfig == nil {
		panic("tls: QUICServer called with nil config")
	}
	c := &Conn{
		config: config.TLSConfig,
		// QUIC supplies the transport; the TLS handshake must not try to
		// access a stream net.Conn. The REALITY-aware server handshake below
		// consumes and emits QUIC crypto events instead.
		isClient: false,
	}
	c.handshakeFn = c.quicServerHandshake
