...
type Config struct {
	...
	// PublicKey and ShortId enable the Pandora-native REALITY client mode.
	// PublicKey is the server's X25519 public key and ShortId is the optional
	// eight-byte client identifier. They are intentionally client-only fields;
	// the server uses PrivateKey and ShortIds above.
	PublicKey []byte
	ShortId   []byte
	// ClientVersion is the three-byte version tuple carried in the REALITY
	// authentication envelope. A nil value uses the Xray-compatible default.
	ClientVersion []byte
...
func (c *Config) Clone() *Config {
	...
	return &Config{
		...
		ShortIds:                            c.ShortIds,
		PublicKey:                           bytes.Clone(c.PublicKey),
		ShortId:                             bytes.Clone(c.ShortId),
		ClientVersion:                       bytes.Clone(c.ClientVersion),
