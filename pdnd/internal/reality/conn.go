...
func (c *Conn) readRecordOrCCS(expectChangeCipherSpec bool) error {
	...
	if realityTraceEnabled {
		...
		realityTracef("[RECORD] typ=%d len=%d head=%x hand=%d hsDone=%v",
			typ, len(data), head, c.hand.Len(), c.isHandshakeComplete.Load())
	}
	switch typ {
	default:
		return c.in.setErrorLocked(c.sendAlert(alertUnexpectedMessage))
...
func (c *Conn) writeRecordLocked(typ recordType, data []byte) (int, error) {
	// 发出去的记录类型序列。读方向早就有 trace，写方向一直没有，结果
	// 「握手多发了一条记录」这类问题在两头都看不见：服务端认为握手正常
	// 结束，客户端只回一个 alert 就静默断开。带上 handshakeLen 是因为
	// 每条握手记录会消耗掉对应的伪装长度，看哪一位归零就知道刚发的是
	// 第几段。默认零开销。
	if realityTraceEnabled {
		fmt.Fprintf(os.Stderr, "[REALITY] write typ=%d len=%d lens=%v\n",
			typ, len(data), c.out.handshakeLen)
	}
	if c.quic != nil {
