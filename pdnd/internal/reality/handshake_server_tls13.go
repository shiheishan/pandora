...
import (
	...
	"github.com/aegispanel/nodeagent/internal/reality/fips140tls"
	"github.com/aegispanel/nodeagent/internal/reality/hpke"
	"github.com/aegispanel/nodeagent/internal/reality/tls13"
...
func (hs *serverHandshakeStateTLS13) handshake() error {
	...
	*/
	...
	// 只有客户端表示接受会话票据时才发这个伪造票据。RFC 8446 4.6.1：
	// 客户端没给 psk_key_exchange_modes，服务端就不得发 NewSessionTicket。
	// Mihomo 的 REALITY 客户端设了 SessionTicketsDisabled，收到就回一个
	// unexpected_message 告警把连接掐掉——握手看着是成功的，第一次读才
	// 炸，且客户端侧不打任何日志。
	//
	// 这里刻意只看客户端的 pskModes，不走 shouldSendSessionTickets()：
	// 那个函数还会检查服务端的 SessionTicketsDisabled，而我们正是靠把
	// 它置位来关掉标准的真票据（见 reality_listener.go）。真票据有
	// 122 字节，而 conn.go 里那段把票据伪装成 application_data 的改写
	// 只动 record[5]/record[6] 两个字节——那是为一字节的合成票据写的，
	// 套到真票据上只会改坏内容，内层类型仍是 handshake，对端看到的就
	// 是一条首字节非法的握手消息。
	if hs.c.out.handshakeLen[6] != 0 && slices.Contains(hs.clientHello.pskModes, pskModeDHE) {
		// The target ticket is only a camouflage length hint. A very small
		// target record cannot contain even the one-byte synthetic ticket plus
		// TLS 1.3's inner content type and AEAD tag; passing it to encrypt would
		// produce a negative padding length and abort an otherwise valid
		// authenticated handshake. Skipping an optional ticket is safe: session
		// resumption is not required for the REALITY data connection.
		cipher, ok := hs.c.out.cipher.(aead)
		if ok && realitySessionTicketRecordFits(hs.c.out.handshakeLen[6], cipher.Overhead()) {
			if _, err := c.writeRecord(recordTypeHandshake, []byte{typeNewSessionTicket}); err != nil {
				return err
			}
		} else {
			hs.c.out.handshakeLen[6] = 0
		}
	}
...
// realitySessionTicketRecordFits reports whether a TLS 1.3 record can carry
// the one-byte synthetic NewSessionTicket marker used by REALITY camouflage.
// handshakeLen includes the five-byte TLS record header.
func realitySessionTicketRecordFits(recordLen, overhead int) bool {
	const syntheticTicketLen = 1
	const innerContentTypeLen = 1
	if recordLen < 0 || overhead < 0 {
		return false
	}
	return recordLen >= recordHeaderLen+syntheticTicketLen+innerContentTypeLen+overhead
}
