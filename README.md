...
本地快照不含 `.env`、密钥、私钥和编译产物（二进制、`node_modules`、`dist`）。测试机运维脚本、安装日志和真实服务器地址放在被忽略的 `ops-local/`，只存在于维护者本机；运维脚本的密钥从 1Password 读取，不写进任何文件。
...
三个网关只监听 `127.0.0.1`，公网访问要在前面放反向代理并配 TLS：
