...
分三层：api.ts 是契约的镜像（zod 写全写严，待补·后端字段一律可选，流量与积压三件照 DASH-01 冻结契约），model.ts 把数据映射成卡片、行与文案（契约各条目的「映射」一行都落在这里，纯函数、全测），其余组件只负责摆放。图表按规范手写，不引图表库；柱高与条宽是仅有的动态 style。
与外框的关系：「需要处理」和侧栏徽标读同一个 GET v1/dashboard/tasks，但查询键分开（侧栏的 schema 只取 count，共用缓存会互相覆盖形状）；卡片与行的链接只在目标页可读时出现，按 modules.ts 判断。

成员清单
index.tsx: 页面入口，按 dashboardAccess 决定哪些卡出现；概览与节点流量两条查询在这里取一次，交给 KPI 与排行共用
api.ts: 八个接口的 zod schema、类型与 useXxx 查询 hook（tasks / backlog / overview / revenue / traffic nodes·users / system status / stats timeseries），刷新间隔与实时 topic 在这里声明；用户流量以节点流量的 snapshot_at 为锚点
model.ts: 纯逻辑——BigInt 安全的 formatBytes、计数 / 百分比 / 时长 / 延迟 / 绝对时间格式化、按权限的 dashboardAccess 与 reachable、需要处理卡片（含通知积压合并与待补·前端的账本漂移卡）、KPI 较昨日、收入趋势与注册活跃的柱数据、备份摘要与系统状态行、流量排行与未归属告警
model.test.ts: model.ts 全部分支与 schema 形状的单元测试（待补字段缺失时的退化、字节必须是字符串、未知 kind 判为不符、只读账号只剩通知积压）
Tasks.tsx: 「需要处理」卡片网格（tasks + 冻结契约的通知积压明细），目标页不可读时卡片不可点
