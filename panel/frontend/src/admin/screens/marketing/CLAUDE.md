...
待补·后端（主线 5bc2f85 时都未实现，后端一第 ⑥ 步）：gift-cards/stats 的 balance_issued（缺时第四格退回「已兑出余额」）、commission/overview 的 total_earned / invited_users（缺时显示「—」）与 scope、commission/config 的 scope（后端没回 scope 时控件隐藏、请求不带，避免 DisallowUnknownFields 整单 400）。
...
parts.tsx: StatStrip 四格统计、Pager 分页、QueryView 加载 / 错误（404 按无权限或不存在）/ 空三态
...
marketing.module.css: 模块公共样式（栈、工具条、统计条、面板、表单网格、分页）
