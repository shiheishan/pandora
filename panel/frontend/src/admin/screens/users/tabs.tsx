 ...
 * [POS]: 用户抽屉的四个资料标签页：画像（累计消费（按币种，R114）/ 订单数 / 邀请人数统计条、用户组分配、显示名 / 邮箱验证 / 风险 / 角色 / 余额 / 邀请人 / 注册 IP / 最近登录 / Telegram）、订阅（全部订阅，流量配额、设备上限 − / + 与「恢复套餐默认」「不限」，按保留规则 2 不出现订阅地址）、设备（D-B-8 已决（5.A.2）方案 A：只有在线台数与上限）、订单（最近 20 单，人工单挂「人工」标识（R114），跳订单页按 user_id 精确筛选）
