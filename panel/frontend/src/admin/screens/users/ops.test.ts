 ...
 * [POS]: admin/screens/users 第 ④ 步的测试，与 model.test.ts（列表与抽屉）并列：用户组删除拦截与「被引用」、批量筛选表单到 BulkFilter 与导出 query、批量生成的前端校验（与 adminops.GenerateUsers 同规则）、接近上限与在线格、重置日志的操作人文案、重置原因、手动重置挑哪条订阅、按邮箱精确匹配；schema 守住 omitempty 键、封闭枚举与「口令只回一次」的形状
 * [PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
 */
import { describe, expect, it } from 'vitest'
import { bulkPreviewSchema, devicesSchema, generatedSchema, resetDoneSchema, resetLogsSchema, resetStatsSchema, type OnlineDevice } from './api'
import {
  bulkFilter,
  exactEmail,
  exportQuery,
...
describe('用户组', () => {
  const g = { users: 0, plans: 0, prices: 0, coupons: 0 }
  it('按后端 409 的先后顺序说明为什么不能删', () => {
...
describe('schema', () => {
  ...
  it('设备：模式只有 loose / strict，last_seen_at 可为 null', () => {
    ...
    expect(devicesSchema.safeParse({ devices: [row], mode: 'strict', grace: 1 }).success).toBe(true)
    expect(devicesSchema.safeParse({ devices: [row], mode: 'kick', grace: 1 }).success).toBe(false)
  })
