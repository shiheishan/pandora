...
NodeOps.tsx: 节点操作：发布配置、复制（目标服务器、复制路由）、迁移（保留规则 5）、启用 / 停用、退役、删除，每项无权限或状态不允许时写明原因
...
PoolsTab.tsx: 节点池三栏卡片（名称 · 节点数 · 状态 | 成员标签跳节点抽屉 | 绑定套餐），新建 / 编辑弹窗，有节点或套餐时删除禁用并说明，其余阻碍靠 409 文案
