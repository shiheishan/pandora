...
分层：schemas（zod；Go 指针字段写 nullable、nil 切片归一成 []）→ queries（读 hook、写后按前缀失效；useCan / useFailure / useIntentKey 暂从 marketing 转出，admin/actions.ts 合入后改引那里）→ logic（纯函数，nodes.test.ts 守住）→ 组件。
...
queries.ts: 查询键前缀 NK、各读 hook（列表挂 nodes.changed）、useInvalidateNodes，转出三件通用 hook
