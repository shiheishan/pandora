...
logic.ts: 节点状态映射（在线 / 离线 / 排空中 / 草稿 / 已停用 / 已退役）与筛选搜索、心跳与地址文案、迁移资格与 409 资产清单、R108 上线资格、R113 上线提示（warnings → 带下一步的普通语气文案）、合法状态边与批量取舍、排序提交项、协议表单模型（敏感字段留空不带 / 显式清空为 null、要清空的列表）、基本信息校验与新建体 / PATCH 差量、路由规则行互转与兜底校验、新规则插在兜底前、出站被引用计数与改名联动、出站行校验、带宽分桶
...
NodeOps.tsx: 节点操作：上线（R108，接入尾段才显示；warnings 用普通语气提示下一步，池没绑套餐是首次搭建的正常状态）、发布配置、复制（目标服务器、复制路由）、迁移（保留规则 5）、启用 / 停用、退役、删除，每项无权限或状态不允许时写明原因
