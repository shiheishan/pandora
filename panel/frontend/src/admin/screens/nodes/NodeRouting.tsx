 ...
 * [OUTPUT]: 对外提供 NodeRouting（节点抽屉「路由」标签）、RuleRows（规则行编辑器，第 ③ 步全局路由页复用）、OUTBOUND_TYPES
 * [POS]: admin/screens/nodes 抽屉的单节点路由（契约待补·前端：后端有、设计缺）：GET / PUT v1/nodes/{id}/routing，全量替换本节点私有出站与规则；规则可以指向 direct / block、本节点私有出站与全局出站（R26，大小写不敏感）；匹配类型只放后端支持的（D-D-1），兜底必须是最后一条启用规则。保存要 node.config.publish，成功后后端通知节点
...
import { MATCH_KINDS, routeToRow, rowsToRoutes, type MatchKind, type RuleRow } from './logic'
...
function RoutingEditor({ nodeId, data }: { nodeId: string; data: Routing }) {
  ...
  const [outbounds, setOutbounds] = useState<OutboundRow[]>(() => data.outbounds.map((o) => ({ tag: o.tag, type: o.type, settings: JSON.stringify(o.settings ?? {}, null, 2) })))
  ...
  const submit = () => {
    ...
    const oErrors: Record<number, string> = {}
    const parsed = outbounds.map((o, i) => {
      const tag = o.tag.trim()
      if (!tag || tag.length > 64) oErrors[i] = '标签 1–64 字'
      else if (['direct', 'block'].includes(tag.toLowerCase())) oErrors[i] = '不能叫 direct 或 block'
      else if (outbounds.some((x, j) => j < i && x.tag.trim().toLowerCase() === tag.toLowerCase())) oErrors[i] = '标签重复'
      let settings: unknown = {}
      try {
        settings = o.settings.trim() ? JSON.parse(o.settings) : {}
      } catch {
        oErrors[i] ??= 'settings 不是合法的 JSON'
      }
      return { tag, type: o.type, settings }
    })
    setRuleErrors(errors)
    setOutErrors(oErrors)
    if (Object.keys(errors).length || Object.keys(oErrors).length) return
    save.mutate({ row_version: data.row_version, outbounds: parsed, routes })
  }
    ...
    <div className={css.stackLg}>
      <section>
        <div className={css.sectionHead}>
          ...
          <span className={css.faint}>本节点的规则排在全局规则之前，自上而下匹配，命中即停</span>
        </div>
      ...
      <section>
        ...
        {outbounds.length === 0 && <div className={css.faint}>没有私有出站。</div>}
        {outbounds.map((o, i) => (
          <div key={i} className={css.outboundRow}>
            <Input label="标签" mono value={o.tag} disabled={!writable} error={outErrors[i]} onChange={(e) => setOutbounds(outbounds.map((x, j) => (j === i ? { ...x, tag: e.target.value } : x)))} />
            <Select label="类型" value={o.type} disabled={!writable} onChange={(e) => setOutbounds(outbounds.map((x, j) => (j === i ? { ...x, type: e.target.value } : x)))} options={OUTBOUND_TYPES.map((t) => ({ value: t, label: t }))} />
            <TextArea label="settings（JSON）" mono rows={2} value={o.settings} disabled={!writable} fieldClassName={css.span2} onChange={(e) => setOutbounds(outbounds.map((x, j) => (j === i ? { ...x, settings: e.target.value } : x)))} />
            {writable && (
              <Button size="xs" variant="ghost" aria-label={`删除出站 ${o.tag || i + 1}`} onClick={() => setOutbounds(outbounds.filter((_, j) => j !== i))}>
                <IconClose />
              </Button>
            )}
          </div>
        ...
        {writable && (
          <Button size="sm" onClick={() => setOutbounds([...outbounds, { tag: '', type: 'shadowsocks', settings: '{}' }])}>
            ＋ 添加出站
          </Button>
        )}
      </section>
...
export function RuleRows({
  ...
  return (
    <div className={css.rules}>
      ...
      {!disabled && (
        <div>
          <Button size="sm" onClick={() => onChange([...rows, { kind: 'domain_suffix', value: '', outbound: 'direct', enabled: true, note: '' }])}>
            ＋ 添加规则
          </Button>
        </div>
      )}
