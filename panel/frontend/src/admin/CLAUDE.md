...
admin.test.ts: 路由规范化与 rest 子路由、读权限表、⌘K 筛选与隐藏、reauth 桥、身份文字映射、强度条、事件条目、tasks schema 与徽标取数、actions 的权限与写失败处理的纯逻辑测试（幂等键本身在 core/intent.test.ts）；界面交互在浏览器里对 dev/mock-api 验收
