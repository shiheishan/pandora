...
管理后台入口（管理后台.dc.html 外壳）。登录态只看令牌：没有令牌是整页 LoginPage，有令牌是 Shell（深色侧栏 + 粘性顶栏 + 页头标签 + 模块内容区）；第 3 阶段的模块页按 modules.ts 的 MODULES 挂进 Shell 的内容区。
...
路由是 hash：#/<模块>[/<标签>]，未知模块或标签由 Shell replace 成规范地址。侧栏入口暂不按权限隐藏（模块与权限码的映射在第 3 阶段按各模块契约补），缺权限的接口回 404，页面当作「无权限或不存在」。
...
modules.ts: 十个模块的标题、分组、标签（「欠费单」按保留规则 6 叫「挂账」），侧栏六组，⌘K 深链；resolveRoute / modulePath 负责规范地址，paletteItems 负责筛选
...
Shell.tsx: 外框编排：Sidebar、顶栏面包屑与 EventsCapsule、页头标题与 ui/Tabs、内容区空状态；⌘K 全局快捷键、规范地址 replace、document.title
Sidebar.tsx: 字标与构建版本号（__APP_RELEASE__）、⌘K 入口、六组导航（工单 / 营销徽标取待补·后端的 GET v1/dashboard/tasks，只在有 ops.dashboard.read 时请求）、向上弹出的账户菜单（主题、改密码、打开门户 ../、退出）
...
CommandPalette.tsx: ⌘K 命令面板，原生 <dialog>，combobox + listbox，↑↓ / ↵ / Esc
...
admin.test.ts: 路由规范化、⌘K 筛选、reauth 桥、身份文字映射、强度条、事件条目的纯逻辑测试；界面交互在浏览器里对 dev/mock-api 验收
