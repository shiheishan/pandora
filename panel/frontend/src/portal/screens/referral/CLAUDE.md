...
设计稿之外：邀请链接改为 /?invite=<code>（/r/ 会撞订阅通配），邀请码 8 位；横幅不写「首单」（门户接口不回计佣范围 scope，修订 R67 只在后台），写「邀请好友付费」；邀请码有上限时说明用量、用满或站点关闭注册时警示；左列补「邀请记录」卡（契约待补·前端，不展示 risk_flag）；提现占位改「支付宝账号 / 银行卡号」，不做 USDT，「1–3 个工作日」改为「审核通过后打款」（后端无此承诺）。
