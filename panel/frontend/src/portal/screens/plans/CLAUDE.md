...
选购套餐（用户门户-03-选购套餐.dc.html 的列表部分；契约门户-03）。两个标签：订阅套餐与流量包，#/plans?tab=packs 直达后者。只展示 CNY 价格（余额与 epay 只有 CNY）；待决 D-E-3 未定，套餐卡的特性只列后端已有事实（流量与重置、设备数、描述），不显示「推荐」。流量包规则按 5.A D-E-1（挂用户、永不过期、可叠加、有生效订阅才消耗），删去设计稿「没有订阅也能买并单独使用」一条。
