...
页面之间的约定地址：#/subs?sub=<订阅 id>（我的订阅选中哪条）；#/plans?tab=packs（流量包标签）；#/checkout?plan=<套餐>&price=<价格>（无订阅新购、同套餐续费、换套餐变更，由结账页判定）、?renew=<订阅 id>[&price=]（续费）、?pack=<流量包>；#/orders/<订单 id>（第 ③ 步订单明细；带 ?paid=1 时是收银台回跳，弹支付确认）；#/messages?tab=announcements（第 ⑤ 步消息页接）。
页面数据只经 common/ 与外框 queries.ts 的查询取，同一接口同一查询键；外框 schema 不够用时在 queries.ts 里写全、外框经 select 取自己的部分（订阅列表即如此），不另起查询键。
...
orders/: 我的订单（门户-04）；第 ② 步只接收银台回跳确认，列表与明细第 ③ 步接入；见 orders/CLAUDE.md
wallet/ referral/ tickets/ messages/ help/ account/: 其余页面目录，index.tsx 默认导出页面组件，目前渲染 Placeholder，按开工说明第 ③–⑥ 步逐个接入
