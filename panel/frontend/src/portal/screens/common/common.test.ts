/**
 * [INPUT]: 依赖 vitest，依赖同目录 traffic / orders / clients / subscriptions 的纯函数与 schema，依赖 ../subs/labels
 * [OUTPUT]: 无（测试文件）
 * [POS]: portal/screens/common 与 subs 文案映射的单元测试：流量摘要与预测、用量柱、到期、订单标题与期限、深链与协议名、主订阅选择、schema 对契约形状（含待补字段缺席）的收放
