...
用户门户入口（用户门户.dc.html 外壳）。未登录是 AuthPage（登录 / 两步注册 / 快捷登录），登录后是 Shell（粘性顶栏、页头、内容区、页脚，< 640 底部标签栏）；第 3 阶段的十个页面按 pages.ts 挂进 Shell 的内容区。门户没有 reauth。
...
pages.ts: 十一个页面的标题与副标题、顶部导航四项（「订单」短于页标题）、头像菜单五项；resolvePage 规范地址，navOwner 让结账页归「选购套餐」，greeting 按时段问候
queries.ts: 外框读接口：site-config 与 appearance（匿名，登录页用）、me、余额（topic orders.changed）、当前套餐名（第一条仍有效的订阅）、可用佣金、未读数（无 SSE，60 秒轮询 limit=1）；schema 只收外框用到的字段；displayName 按契约映射
...
Shell.tsx: 顶栏（字标、导航、≥ 960 余额胶囊、铃铛未读角标、头像菜单：用户名 + 套餐徽标 + 邮箱、五项带余额 / 佣金提示、深色模式开关、退出）、页头、空状态内容区、页脚、< 640 五格标签栏（「我的」受控打开头像菜单）；连门户 SSE，只驱动查询失效
*.module.css: 各组件同名样式，断点只用规范的 960 与 640
portal.test.ts: 页面路由与导航归属、邀请码取用与查询串清理、快捷登录令牌识别、主题令牌白名单、用户名映射的纯逻辑测试；界面交互在浏览器里对 dev/mock-api 验收
