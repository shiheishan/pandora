...
路由是 hash：#/<页面>[/<rest>…]，未知页面由 Shell replace 成概览，合法页面后的段作为 rest 交给页面（#/orders/<订单号>、#/tickets/<id>），查询串（#/checkout?...）不受规范化影响。
...
queries.ts: 外框读接口：site-config 与 appearance（匿名，登录页用）、me、余额（topic orders.changed）、订阅列表（全字段 schema，外框与页面共用 ['portal','subscriptions'] 一个键，套餐徽标经 select 取 pickPrimary 那条，与概览主卡一致）、可用佣金、未读数（无 SSE，60 秒轮询 limit=1）；除订阅外 schema 只收外框用到的字段；displayName 按契约映射
