...
src/admin/: 管理后台入口与外框：登录页、深色侧栏 + 顶栏 + 页头标签、⌘K、实时事件、改密码、常驻 reauth 对话框（接到 api 的 requestReauth）；模块页第 3 阶段接入；见 src/admin/CLAUDE.md
src/portal/: 用户门户入口与外框：登录 / 两步注册 / 快捷登录、顶栏导航与头像菜单、< 640 底部标签栏、邀请与快捷登录链接、外观令牌；页面第 3 阶段接入；见 src/portal/CLAUDE.md
src/shell/: 两个入口共用的外框底座：每入口一份运行时（令牌 + api + QueryClient）、登录态、退出、实时事件、Logo；见 src/shell/CLAUDE.md
src/env.d.ts: 构建期常量 __APP_RELEASE__ 的类型声明
dev/: 只在 vite serve 存在的假后端 mock-api.ts，本机无 PostgreSQL 时在浏览器里走通外壳流程；见 dev/CLAUDE.md
