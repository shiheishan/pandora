...
src/core/: 与界面无关的底层：主题引导与状态、唯一 HTTP 出口 api.ts（相对 v1/ 路径、Bearer、错误信封、幂等键、reauth 重放、非 JSON 响应的 requestRaw）、文件下载 download.ts、令牌存储、fetch 流 SSE、react-query 客户端与实时失效、hash 路由、金额与相对时间格式化；见 src/core/CLAUDE.md
...
tests/mock-api.test.ts: 假后端守卫——把 mockApi 挂到本地 HTTP 服务上，验证外壳接口、模块分发、权限 404 先于 reauth、reauth 不消耗幂等键、同键重放与换请求 409、门户外框读接口与快捷登录往返；营销假接口的礼品卡掩码、一次性导出（非 JSON 重放不带 Content-Disposition）与未知字段 400
