...
plans-store.ts: 套餐假接口的数据与规则：五个种子套餐（沿用 users.ts 的固定套餐 id 与用户组 id，节点池沿用 nodes-infra.ts 的池 id，在线节点数取 nodes-infra.ts 的 activeNodesInPool，与节点池列表同口径）、列表行与详情形状、与 Go 同键名同文案的校验、发布前置条件、销售开关、按 DisallowUnknownFields 拒绝未知字段
