...
- **修订 R34（2026-09-24，第 2 阶段 e687e4d）**：前端已按「先请求、按需弹框」实现，reauth 对话框常驻在 admin 入口、接在 api 客户端上，页面不用自己处理；用户在框里取消时原请求以 `reauth_required` 失败，页面对这个错误码应静默忽略（不弹 Toast、不报错）。框里口令错只在框内提示，不触发登出。
...
- **修订 R38（2026-09-24，后端一 91738d3）**：`reason` 新增取值 `plan_change`（变更套餐清零），筛选与 `by_reason` 同步认它；「方式」列显示为「变更套餐」。
...
- 请求：无
- 响应：200 `{ timezone: string(IANA 名，如 "Asia/Shanghai") }`（取 `tenants.timezone`）
- 错误：无特有
...
- 响应：200 `{ timezone: string }`
- 错误：422 `fields.timezone`「不是有效的时区」
- 副作用：写审计（旧值 → 新值）
- 需迁移：见 R49（`tenants.timezone` 默认值改为 `Asia/Shanghai`，存量仍为 `UTC` 的租户一并改掉）
...
- 表 `subscription_usage_daily` 按订阅 × 天持续增长，**不做清理**（用户 2026-09-24 定：先留着、记录在案；接口最多只读 93 天）。
- **修订 R50（2026-09-24 用户定案，后端二第 ⑤ 步实现）**：用户 `timezone` 为 `'UTC'`（即默认值、从未设置）时视同未设，改用站点时区；站点时区默认 `Asia/Shanghai`，后台可改（`POST v1/settings/site`）。其余同 R48。后台收入趋势本来就按站点时区切日，随之生效。
- 状态：待补·后端
...
- **修订 R36（2026-09-24，后端一 91738d3、077eb27）**：已实现（迁移 00071）。响应另加 `proration_credit: int`、`balance_refund: int`；**升级与降级的订单 kind 都是 `upgrade`**（后台订单列表的 `downgrade` 取值不会出现）。应付为 0 时当场履约（status=fulfilled），差额以 `plan_change_refund` 分录退进余额。履约在原订阅上换套餐，订阅地址不变，新周期从当天按新套餐起算，已用流量清零并记重置日志（reason `plan_change`），流量包余额不受影响；新套餐没有的配额指标置为不限量。错误同 preview，另 409「余额不足」。
- 状态：待补·后端
...
| R34 | 2026-09-24 | 第 2 阶段 e687e4d | reauth 取消时请求以 `reauth_required` 失败，页面静默忽略；对话框由入口统一接管 |
| R35 | 2026-09-24 | 后端一 91738d3 | change-plan/preview 已实现：加 balance_refund、direction 按 total 定、允许降级、新增三种 409 |
| R36 | 2026-09-24 | 后端一 91738d3、077eb27 | change-plan 下单已实现：升降级 kind 都是 upgrade，0 元当场履约并退余额，原地换套餐 |
...
| R116 | 2026-09-25 | 前端收尾 ⑤ | 前端已删英文→中文文案映射，4xx 响应里给用户看的 `message` 一律中文（`code` 与形状不变）；首例人工开单缺渠道的「unknown payment provider」 |
