...
**进度**：①–⑦ 已验收合入。第 5 阶段：⑧ `8993672` + `7e5ec8c`（合并 49f7ba9；PG18 233 PASS / 0 SKIP / 0 FAIL，NativeCore、panel-smoke 全绿；迁移 00095；R117）已验收合入。**⑨–⑫ 在做，另追加 ⑬（前端挂账枚举）**，见补充事项。迁移剩 00096–00098。
...
- **第 5 阶段（2026-09-26，新会话接力）**：先读 `phase3-common.md` 第 12 节（与重构会话的文件分界）。用户定了修下面五件，都是协调会话调查过、根因已明的问题。⑧ 单独一个提交、单独报告（牵涉钱）；⑨–⑫ 每步一个提交，全部做完一起报告。每个提交推送后看三组 CI，PG18 必须 0 SKIP；本机全量用 `go test -p 1 -count=1 -timeout 15m ./...`。
  ...
  - **⑬ 前端挂账枚举（R117，⑨–⑫ 之后做，越界授权改前端这几处）**：`panel/frontend/src/admin/screens/billing/schemas.ts` 的 `LATE_KINDS` 加 `ineligible_subscription`，挂账页文案照现有两种的写法（建议「订阅已结束后到账 · {order_no}」），假后端挂账数据加一条这类行、假后端测试覆盖它能被页面 schema 解析并能转入余额；标记已付两种 409 原样 Toast（页面若有英文映射残留就删）。在你的 worktree 里 `cd panel && make frontend-check`，再 `make frontend-embed && go test -count=1 ./web/... ./internal/platform/webapp/...`，最后 `git checkout -- web && git clean -fdXq web`。冒烟 `tests/smoke/seed.ts` 不用改（这类行只有手工 SQL 能造）。一个提交，随 ⑨–⑫ 一起报告。
