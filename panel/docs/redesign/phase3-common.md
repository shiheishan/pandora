...
- 真机相关（测试机上的 aegis-nodeagent 定性与迁移、删 `nodeagent/`、把测试机登记进 `~/ai/servers`）**本阶段不做**。**2026-09-26 用户定：旧测试机、验证机全部退役删除，不再定性**；`nodeagent/` 目录与 `panel/deploy/systemd/` 里它的单元和 override 已删，真机测试以后换新机器再议。
