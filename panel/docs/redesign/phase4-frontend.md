...
**进度**：①（合并 283797d）、②（合并 bb0480a）、③ + ④ + 补丁（合并 873a6f1）、⑤ `d9cbd6c`（合并 0dab1f1；CI：NativeCore 36202928127、PG18 36202928119、panel-smoke 36202928120 全绿；主线合并后 frontend-check 631 用例、冒烟 tsc、嵌入契约通过）已验收合入。**前端收尾五步全部完成，暂无待办。**

补充事项（与上文冲突时以这里为准）：
- 契约修订已到 R117。
- ⑤ 的验收结论：八项都认可。删掉英文映射与 `billing/failure.ts` 包装、直接用 `useFailure`；`paid_total` 不再收；`manual`、`coupon`、`last_seen_at` 收紧为必填；上线 warnings 用普通提示样式并带下一步；共享层没动。新测试断言字段而不用页面 zod schema（模块依赖 tsx 进不了 node 侧类型检查）可以接受，真实形状由联调冒烟用页面 schema 兜住。「人工」标签在 160 宽的订单号列里换行、只让人工单那行变高，可以接受。
- 你报的 Go 里剩下的英文（人工开单缺 offline 渠道回「unknown payment provider」）已写成 R116，连同其余英文 4xx 文案一起交给后端四 ⑥，前端不用再加映射。
- **第 ⑤ 步（R114，后端三 ⑤ 已合入主线 bd0b7ff）**：先 `git merge feat/panel-redesign`。以前按「后端没做」降级写的七处，现在后端都已补上，schema 收紧为必填（真后端已上线），假后端同步形状，逐条对照契约里的「修订 R114」行：
  ...
  8. 节点「上线」的 `warnings` 提示（联调冒烟 ④ 提醒）：首次搭建时池还没绑套餐，上线一定会带「所在节点池没有绑定任何套餐」——这是正常流程（先上线再发布套餐），Toast 用提示语气而不是错误样式，文案里带上下一步（去套餐页把池绑上并发布）。
