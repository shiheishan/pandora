...
**进度**：①（合并 24c2174，R106）、②（合并 cb861f6，R107）、③（合并 93b5b65，R110）、④（合并 dcd9764，R112）、⑤ `e7c72d0` + `ab7e5a3` + `9a24e87` + `9861e10`（改写后的提交号，原 `fe0e69a` / `5f240c4` / `40d9da7` / `220a9dd`；合并 bd0b7ff；PG18 run 36199472526：231 PASS / 0 SKIP / 0 FAIL，panel-smoke 36199472507 全绿；契约 R114）已验收合入。**后端三五步全部完成，本会话不再开工。** 迁移用到 00090，00091–00092 未用。联调冒烟查出属于本范围的问题时，协调会话追加在这里，用户在同一 worktree 另开会话。

补充事项（与上文冲突时以这里为准）：
- 契约修订已到 R115。
- ⑤ 的验收结论：工单详情与队列同口径、`paid_totals` 按币种且 `paid_total` 暂留（协调会话定为弃用、不排期删除）、取消与凭证号文案收进一个 `releaseHTTPError`、订单行 `manual` 与详情同口径、零元单（赠送、全额抵扣、零元续费与变更）提交后通知节点、两个网关改走 `NotifyUsersChanged`（public 建不带签名器的 Service 只发通知）、`last_seen_at` 放认证中间件同事务节流 5 分钟、开关一致性单测（变异验证过）与存量 / 新建租户开关集合 PG18 比对、扫描按实际插入行计数、R76 `CouponFace` 与 R81 `scope`，都认可，已写成 R114。续费「周期走完从付款时刻起算」是顺带查出的真缺陷，修得对，也写进了 R114。
- 误提交的二进制：协调会话用 `git commit-tree` 原样复用各提交的树重建了分支（内容零差异、新历史里没有 `panel/aegis-admin`），用户执行了强推。本机 `be3-backup` 分支指向旧的 `220a9dd`，合入后可以删。
- 你报告的剩余风险记下了：订阅状态若被 SQL 手工改成 `expired` 或导入时带着它，续费单待支付期间付款回调会被状态机拒绝并一直重试。产品路径里没有写 `expired` 的地方，仓库里也没有导入器，**本阶段不修**，已报用户。
