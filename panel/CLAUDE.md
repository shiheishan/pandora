...
frontend/: 面板前端源码，React + TypeScript + Vite 一个工程两个入口（--mode admin|portal 分两次构建到 dist/{admin,portal}），按设计稿重写：底座、组件库与两边外框（登录态、reauth 对话框、实时事件）已就绪，模块页在第 3 阶段接入；本机无数据库时 dev 挂假后端；见 frontend/CLAUDE.md
