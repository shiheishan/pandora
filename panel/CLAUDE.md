...
migrations/: goose SQL 迁移 00001–00068 按序号递增，不变量以触发器与约束落在这里；00067 删除 21 张无依赖孤儿表且 Down 拒绝回滚；00068 补 marketing.coupon.read 与 marketing.commission.write 两个权限码并按原受众授予；RESERVED-TABLES.md 登记仍保留、Go 从不引用的 16 张表及各自的锁定原因，由 platform/db 的 schema 契约测试守住；frozen-client-auth/ 为冻结的 CLIENT-AUTH 迁移，带 README
