...
migrations/: goose SQL 迁移按序号递增，已合入主线到 00067；第 3 阶段各会话按号段编号（后端一 00068 起：00068 补 marketing.coupon.read 与 marketing.commission.write 两个权限码并按原受众授予，00069 建 gift_card_batches（租户 RLS、仅可一次打导出标记的守卫触发器），存量批次按码回填且一律记为已导出，码的 batch_id 加复合外键；后端二 00074 起：00074 注册验证码模板种子、00075 内置主题换成「默认 · 纸白」、00076 流量余额移出变更通知、00077 风控聚类复核 ip_cluster_reviews、00078 工单快捷回复 ticket_macros、00079 帮助反馈 content_page_feedback（三表均租户 RLS）、00080 审计 auth_context、00081 webhook 投递耗时、00082 节点 country_code 与服务端令牌签发时间 / 签发人（列级授权）），合并时由协调会话压紧，不变量以触发器与约束落在这里；00067 删除 21 张无依赖孤儿表且 Down 拒绝回滚；RESERVED-TABLES.md 登记仍保留、Go 从不引用的 16 张表及各自的锁定原因，由 platform/db 的 schema 契约测试守住；frozen-client-auth/ 为冻结的 CLIENT-AUTH 迁移，带 README
