...
linelimit_test.go: 行数守卫 TestGoFilesStayWithinLineLimit：从 module 根遍历全部 .go（含测试，跳过隐藏目录与 node_modules），超 800 行（按 wc -l 数换行）即红；lineLimitExemptFiles 逐个登记纯挪动拆不开的文件（现为 idempotency_pg18_test.go、order_release_pg18_test.go 两个单函数超长的 PG18 测试），豁免的文件不存在或已降到 800 行以内也红。pdnd 根的 linelimit_test.go 是同一道守卫（豁免两个 fork 目录）
