...
*_test.go: 认证、限流（严格模式）、重认证错误码、超时与流式、降级开关、幂等（认领、业务完成、PG18 并发与接管）测试；switch_seed_test 钉住建租户触发器种的开关与代码读取的开关一一对应；tenant_guard_test 在产品代码出现建租户或 Tenant 不再恒定注入默认租户时失败，提示先扩 app.seed_tenant_defaults；idempotency_pg18_test.go 由 run-pg18-gates.sh 跑
