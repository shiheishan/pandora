...
func TestLedgerReservationLeasePlanCommitExactRetryAndLock(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(rootPath, RecordName))
	if err != nil {
		t.Fatal(err)
	}
...
func TestLedgerReservationLeaseCASConflictDoesNotOverwrite(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(rootPath, RecordName), foreignBytes, 0400); err != nil {
		t.Fatal(err)
	}
	if _, _, err := lease.commit(); err == nil {
		t.Fatal("CAS conflict overwrote foreign ledger")
	}
	after, err := os.ReadFile(filepath.Join(rootPath, RecordName))
	...
	for _, divergent := range []bool{false, true} {
		t.Run(fmt.Sprintf("divergent_%v", divergent), func(t *testing.T) {
			...
			if divergent {
				...
				if _, statErr := os.Stat(filepath.Join(rootPath, RecordName)); !errors.Is(statErr, os.ErrNotExist) {
...
func TestLedgerReservationLeaseRejectsLockBasenameReplacement(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(rootPath, RecordName)); !errors.Is(err, os.ErrNotExist) {
