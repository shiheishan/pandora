...
func readCurrentLedger(directoryFD int) (*ca42authority.Ledger, error) {
	data, _, err := readRootOwnedRegularAt(directoryFD, RecordName, ca42authority.MaxLedgerBytes, nil)
...
func atomicReplaceLedger(directoryFD int, data []byte) error {
	...
	if err := unix.Renameat(directoryFD, candidateName, directoryFD, RecordName); err != nil {
