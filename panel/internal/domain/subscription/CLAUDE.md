...
成员清单
service.go: 订阅凭据解析、节点资格查询 listEligibleNodesTx（经节点池连接套餐授权、服务状态、服务器就绪、协议稳定、至少心跳过一次；无池节点不下发）、门户节点预览、DeliveryState（含无池节点「未划入节点池，不服务任何用户」）、用户自助换链接 Rotate
...
*_test.go: 渲染与资格契约测试；node_preview_pg18_test.go 为资格规则的 PG18 集成测试（run-pg18-gates.sh 的 node_preview 域），usage_daily_pg18_test.go 为按日用量读模型的 PG18 集成测试（usage_daily 域），delivery_pg18_test.go 在同一份夹具上对照节点用户列表、订阅下载、门户预览三处交付集合（delivery 域，与 api/admin 同域），同包三域靠精确过滤互不拉入
