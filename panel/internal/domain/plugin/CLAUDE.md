...
成员清单
hooks.go: 事件白名单 Events（EventInfo，小写 name / desc）、钩子列表带近 7 天 sent_count_7d、Service（钩子增删查，保存时 timeout_ms 500–30000 与 max_attempts 1–10 越界回 422（R93）、Dispatch / StartScanner 投递、Deliveries 投递记录、TestHook 同步测试）；每次尝试经 timedPost 量往返耗时，落 plugin_hook_deliveries.last_duration_ms（00081），没发出去的尝试记 NULL
