...
成员清单
service.go: Service 与全部工单用例：创建、用户读写与关闭、客服队列 / 详情 / 回复 / 指派 / 改状态、超时升级；队列 status 可逗号多值并回 last_message_author_kind（跳过内部备注），详情回 user_active_plan，人工升级把优先级提到至少 high；两侧都回 closed_reason，用户详情回 related_order
