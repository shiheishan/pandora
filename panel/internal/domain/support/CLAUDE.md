...
成员清单
service.go: Service 与全部工单用例：创建、用户读写与关闭、客服队列 / 详情 / 回复 / 指派 / 改状态、超时升级；队列 status 可逗号多值并回 last_message_author_kind（跳过内部备注），详情回 user_active_plan，人工升级把优先级提到至少 high；两侧都回 closed_reason 与 related_order，客服详情的 message_count / last_reply_at 与队列同口径（R75）；客服非内部回复经 SetReplyNotifier 注入的 ReplyNotifier（notify.Service 实现，接口不反向依赖 notify）在同一事务里给提单人排 ticket.replied，变量 subject、去重键 ticket-replied:<消息 id>，service 类别按偏好过滤，提交后 Kick（R115）；admin 网关装配时接上
