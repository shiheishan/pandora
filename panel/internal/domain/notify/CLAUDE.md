...
scan.go: 到期 / 流量 / 支付成功三类扫描入队，StartScanner 循环（Kick 只派发不扫描）；流量预警的可用量 = 套餐本期额度 + 用户流量包剩余（traffic_pack_grants）
...
*_test.go: 单元测试（address_test.go 守验证码模板种子与 Kick 不阻塞）；scan_pg18_test.go 由 run-pg18-gates.sh 的 notify 域跑
