...
nodefabric/: 节点接入与配置下发（PRD 第 8–9 章），18 文件；protocol_secrets.go 让节点 PATCH 保留读接口抹掉的敏感键（R78）；node_retire.go 一步退役；usage_daily.go 在流量上报事务内累加按日用量（00072）；enrollment、node/server admin、node_identity 凭据视图、service；见 nodefabric/CLAUDE.md
