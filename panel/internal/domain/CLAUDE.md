...
billing/: 订单、支付与复式账本，18 文件；借贷配平与回调幂等在此编排；commission_available.go 是「可用佣金 = 账本余额 − 未过账在途提现」的唯一口径，提现申请与转余额在同一把科目锁下共用
...
nodefabric/: 节点接入与配置下发（PRD 第 8–9 章），15 文件；enrollment、node/server admin、node_identity 凭据视图、service；见 nodefabric/CLAUDE.md
