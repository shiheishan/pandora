// [INPUT]: 依赖 plan_change_quote.go 的剩余价值折算、order_holds.go 的预留父节点与余额冻结、coupon.go 的 applyCoupon/redeemCoupon、renewal.go 的 captureZeroPaySubscriptionOrder、provision.go 的 initQuotaBalances、service.go 的 addInterval、traffic_reset.go 的 LogTrafficReset，依赖 middleware 幂等声明、platform/audit、platform/idempotencybind、domain/plugin
