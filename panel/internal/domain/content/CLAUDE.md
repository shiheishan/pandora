...
成员清单
service.go: Service 与 Page 模型；后台 ListAdmin / GetAdmin / PublishVersion / Archive，门户 ListVisible / GetVisible 与唯一的可见性查询 visible
...
*_test.go: service_test.go 单元测试；content_pg18_test.go 与 feedback_pg18_test.go 为 PG18 集成测试（run-pg18-gates.sh 的 content 域）
