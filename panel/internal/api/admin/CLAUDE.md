...
server.go / pools.go: 服务器（物理宿主）读写与状态、节点分组（列表带组内节点 members 与绑定套餐名 plan_names；套餐版本换绑池提交后发租户级 node.users.changed）
...
events.go: 管理端 SSE
*_test.go: 路由契约与守卫（router_contract、security_guards、step4_test、step5_test 与四份 AST 契约，order_pack_routes_test 另用真实注册函数证明未重认证到不了幂等与处理器；源码级契约经 router_source_test.go 读全部 router*.go）、权限字典契约、处理器单测；*_pg18_test.go 为 PG18 集成测试，announcement、node_config 与 delivery（delivery_pg18_test.go：交付集合变化后通知节点、节点列表交付判定与节点用户列表同口径，与 domain/subscription 同域）三个域同包，run-pg18-gates.sh 用精确 -run 过滤分开；phase4_pg18_test.go 归 announcement 域，守节点 PATCH 保留敏感键（R78）与钩子数值越界 422（R93）
