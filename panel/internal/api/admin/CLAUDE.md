...
events.go: 管理端 SSE
*_test.go: 路由契约与守卫（router_contract、security_guards、step4_test、step5_test 与四份 AST 契约，order_pack_routes_test 另用真实注册函数证明未重认证到不了幂等与处理器；源码级契约经 router_source_test.go 读全部 router*.go）、权限字典契约、处理器单测；*_pg18_test.go 为 PG18 集成测试，announcement 与 node_config 两个域同包，run-pg18-gates.sh 用精确 -run 过滤分开；phase4_pg18_test.go 归 announcement 域，守节点 PATCH 保留敏感键（R78）与钩子数值越界 422（R93）
