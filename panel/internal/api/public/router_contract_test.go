...
// 路由表里两者都在，而 /assets/x 这种两段路径必须落到静态下发而不是订阅处理器。
func TestPublicRouterServesEmbeddedAppBesideSubscriptions(t *testing.T) {
	...
	for _, path := range []string{"/assets/index-abc.js", "/assets/missing.js", "/assets/nested/chunk.mjs"} {
		rctx := chi.NewRouteContext()
		if !routes.Match(rctx, http.MethodGet, path) {
			t.Fatalf("GET %s matches no route", path)
		}
		if got := rctx.RoutePattern(); got != "/assets/*" {
			t.Fatalf("GET %s routed to %q, want /assets/*", path, got)
		}
	}
	rctx := chi.NewRouteContext()
