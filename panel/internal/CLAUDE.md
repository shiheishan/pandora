...
成员清单
api/: 三个域网关的 HTTP 路由与处理器，admin 43 文件、public 15 文件、node 3 文件；见 api/CLAUDE.md
domain/: 13 个业务域包共 101 个非测试 Go 文件，billing、nodefabric、dbbackup 最重；见 domain/CLAUDE.md
middleware/: auth.go 四域独立 HMAC 令牌校验与租户注入；idempotency.go 数据库持有的幂等键声明、业务完成绑定与资源绑定；middleware.go 限流、超时、权限与重认证门（RequireRecentReauth 拒绝回 403 reauth_required，前端据此弹框后以原幂等键重放）、公共链装配；*_test.go 含 PG18 幂等、严格限流与重认证错误码测试
